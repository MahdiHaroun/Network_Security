...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self):
        try:
            ...
            self.mongo_client = pymongo.MongoClient(uri)  # call database
            collection = self.mongo_client[database_name][collection_name]  # call collection
            df = pd.DataFrame(list(collection.find()))  # fetch all the data from collection and convert into dataframe
            if "_id" in df.columns.to_list():
                df = df.drop(columns=["_id"], axis=1)  # drop the _id column as it is not required for training

            df.replace({"nan": np.nan}, inplace=True)  # replace "nan" string values with np.nan
            return df
        except Exception as e:
            raise NetworkSecurityException(e, sys)
