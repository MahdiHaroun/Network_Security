...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact: # return type is DataValidationArtifact
        try:
            ...
            status = self.validate_is_numerical_column_exist(dataframe=train_dataframe)
            if not status:
                raise Exception(f"Train dataframe does not have all numerical columns")
            status = self.validate_is_numerical_column_exist(dataframe=test_dataframe)
            if not status:
                raise Exception(f"Test dataframe does not have all numerical columns")
            # check data drift 
